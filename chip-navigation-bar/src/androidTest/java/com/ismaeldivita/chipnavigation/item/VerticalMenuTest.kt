...
class VerticalMenuTest {
    ...
    fun testSelection() {
        menu(rule) {
            ...
            noItemSelected(R.id.메인화면, R.id.discover, R.id.search, R.id.settings)
    ...
    fun testCollapseBehavior() {
        menu(rule) {
            loadMenuResource(R.menu.test_menu, VERTICAL)
            expand()
            item(R.id.메인화면) { isTitleDisplayed(true) }
            collapse()
            item(R.id.메인화면) { isTitleDisplayed(false) }
            expand()
            item(R.id.메인화면) { isTitleDisplayed(true) }
        }
    }
